(function ($) {
  ...
  //       totalPriceElement.attr("data-original-price")
  //     ); // Tổng tiền ban đầu

  //     if (!voucherCode) {
  //       $("#voucherMessage").text("Vui lòng nhập mã voucher").show();
  //       return;
  //     }

  //     // Kiểm tra mã voucher
  //     if (vouchers[voucherCode]) {
  //       const discountPercent = vouchers[voucherCode];
  //       const discountedPrice =
  //         originalTotalPrice * (1 - discountPercent / 100);

  //       // Cập nhật giao diện
  //       totalPriceElement.text(
  //         formatCurrency(discountedPrice.toFixed(2) + " đ")
  //       );
  //       totalPriceElement.attr(
  //         "data-cart-total-price",
  //         discountedPrice.toFixed(2)
  //       );
  //       $("#voucherMessage")
  //         .text("Mã voucher hợp lệ!")
  ...
  if (params.has("category")) {
    ...
    factories.forEach((category) => {
      $(`#categoryFilter .form-check-input[value="${category}"]`).prop(
        "checked",
        true
      );
    });
  ...
  $(".btnAddToCartHomepage").click(function (event) {
    ...
    $.ajax({
      ...
      error: function (response) {
        alert("có lỗi xảy ra, check code  :v");
  ...
  $(".btnOrderNowDetail").click(function (event) {
    const productId = $(this).attr("data-product-id");
    const quantity = $("#cartDetails0\\.quantity").val();

    window.location.href = `/order-info?productId=${productId}&quantity=${quantity}`;
  });
